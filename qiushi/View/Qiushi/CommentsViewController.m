...
- (void)viewDidLoad
{
    ...
    _list = [NSMutableArray array];//[SqliteUtil queryCommentsById:self.qs.qiushiID];//
...
- (UITableViewCell *)tableView:(UITableView *)tableview cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableview == self.tableView) {
        ...
        cell.txtfloor.text = [NSString stringWithFormat:@"%d楼",cm.floor];
        //设置用户名
...
-(CGFloat) getTheCellHeight:(int) row
{
    ...
    // 计算出长宽
    CGSize size = [content sizeWithFont:font constrainedToSize:CGSizeMake(contentWidth, 2200) lineBreakMode:UILineBreakModeTailTruncation];
    CGFloat height = 53 - 21 + size.height;
    // 返回需要的高度
    return height;
}
